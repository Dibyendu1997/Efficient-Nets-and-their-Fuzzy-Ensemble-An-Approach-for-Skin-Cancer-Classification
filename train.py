...
def parse_args():
    ...
    parser.add_argument('--model-dir', type=str, default='./weights_without_reward_b5')
    parser.add_argument('--log-dir', type=str, default='./logs_without_reward_b5')
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, mel_idx):
    ...
    print("length of validation train data",len(df_train))
    print("length of validation data",len(df_valid))
...
#         scheduler_warmup.step(epoch - 1)
        ...
        val_loss, acc, roc_auc = val_epoch(model, valid_loader, mel_idx, is_ext=df_valid['is_ext'].values)

        content = time.ctime() + ' ' + f'Fold {fold}, Epoch {epoch}, lr: {optimizer.param_groups[0]["lr"]:.7f}, train loss: {train_loss:.5f}, valid loss: {(val_loss):.5f}, acc: {(acc):.4f}, roc_auc: {(roc_auc):.6f}.'
        print(content)
...
if __name__ == '__main__':
    ...
    elif args.enet_type == 'vit_large_patch32_384':
