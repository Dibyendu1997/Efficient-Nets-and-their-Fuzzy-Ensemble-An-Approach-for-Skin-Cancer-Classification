...
def parse_args():
    ...
    parser.add_argument('--latent-dim', type=int, default=1000ssssssss)
    ...
    with torch.no_grad():
        for (data,target) in tqdm(test_loader):
            if args.use_meta:
                ...
                for model in models:
                	output_dict = model(data, meta)
                	l = output_dict['logits']
                	probs += l.softmax(1)
                    #for I in range(args.n_test):
                        #output_dict = model(get_trans(data, I), meta)
                        #l = output_dict['logits']
                ...
                for model in models:
                    for I in range(args.n_test):
                        output_dict = model(get_trans(data, I))
                        l = output_dict['logits']
                        probs += l.softmax(1)
