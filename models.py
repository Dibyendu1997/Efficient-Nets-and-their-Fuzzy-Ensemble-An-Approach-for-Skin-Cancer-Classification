...
class ViTBase16(nn.Module):
    def __init__(self, enet_type, out_dim, n_meta_features=0, n_meta_dim=[512, 128], pretrained=False):
        ...
        self.enet = timm.create_model(enet_type, pretrained=False)

        self.enet.head = nn.Linear(self.enet.head.in_features, 1024)
    ...
    def forward(self, x, x_meta=None):
        ...
        if self.n_meta_features > 0:
            ...
            #print("meta shape",x_meta.shape)
            x = torch.cat((x, x_meta), dim=1)
            print("x + x_mata shape",x_meta.shape)
        for i, dropout in enumerate(self.dropouts):
            if i == 0:
                out = self.myfc(dropout(x))
            else:
